...
export function ProfilePage() {
  ...
  // BACKEND DAN MA'LUMOTLARNI TORTISH
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      // 4. Reytingdagi o'rni (Undan ko'p XP yig'ganlar soni + 1)
  ...
  // AVTOMATIK NISHONLAR (BADGES) LOGIKASI
  const allBadges = [
    { id: 1, name: "Ilk Qadam", description: "Birinchi darsni muvaffaqiyatli tugatdingiz", icon: "BookOpen", earned: userStats.lessons > 0 },
    { id: 2, name: "Yosh Yurist", description: "Birinchi amaliy holatni tahlil qildingiz", icon: "Scale", earned: userStats.cases > 0 },
    { id: 3, name: "Olovli Yosh", description: "Ketma-ket 3 kunlik seriya qildingiz", icon: "Flame", earned: (userData.streak || 0) >= 3 },
    { id: 4, name: "Bilimdon", description: "Jami 5 ta darsni muvaffaqiyatli tugatdingiz", icon: "Brain", earned: userStats.lessons >= 5 },
    { id: 5, name: "Adolat Himoyachisi", description: "Jami 5 ta holatda to'g'ri qaror qabul qildingiz", icon: "Shield", earned: userStats.cases >= 5 },
    { id: 6, name: "Chempion", description: "Reytingda kuchli uchtalikka (Top-3) kirdingiz", icon: "Award", earned: userStats.rank <= 3 && userStats.rank > 0 },
  ];

  const earnedBadges = allBadges.filter(b => b.earned);
  const lockedBadges = allBadges.filter(b => !b.earned);

  // HISOBLASH LOGIKALARI
  ...
  const stats = [
    { label: "Darslar", value: userStats.lessons, icon: BookOpen, color: "text-blue-500 dark:text-blue-400", bg: "bg-blue-100 dark:bg-blue-900/30" },
    { label: "Holatlar", value: userStats.cases, icon: Scale, color: "text-purple-500 dark:text-purple-400", bg: "bg-purple-100 dark:bg-purple-900/30" },
    { label: "Kunlik seriya", value: `${userData.streak || 0} kun`, icon: Flame, color: "text-orange-500 dark:text-orange-400", bg: "bg-orange-100 dark:bg-orange-900/30" },
    { label: "Reyting", value: `#${userStats.rank}`, icon: Trophy, color: "text-yellow-600 dark:text-yellow-400", bg: "bg-yellow-100 dark:bg-yellow-900/30" },
  ];

  const creationDate = new Date(userData.created_at).toLocaleDateString('uz-UZ', { year: 'numeric', month: 'long', day: 'numeric' });
      ...
      >
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-gray-950 dark:text-white">Shaxsiy Kabinet</h1>
          </div>
          <p className="text-muted-foreground dark:text-slate-400 mt-1 text-base md:text-lg">O'quv jarayoni va yutuqlaringiz markazi</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Card className="overflow-hidden border-none shadow-xl bg-white/70 dark:bg-slate-800/70 backdrop-blur-lg relative transition-colors duration-300">
            ...
            <CardContent className="pt-10 pb-8 relative z-10">
              <div className="text-center">
                ...
                <p className="text-sm font-medium text-blue-600 dark:text-blue-300 bg-blue-100 dark:bg-blue-500/20 px-3 py-1 rounded-full inline-block mt-1 mb-6">Kelajak Huquqshunosi</p>
                ...
                <div className="grid grid-cols-3 gap-2 border border-gray-100 dark:border-slate-700/50 rounded-2xl p-4 bg-gray-50/50 dark:bg-slate-900/50">
                  <div className="text-center">
                    ...
                    <p className="text-[10px] font-semibold text-muted-foreground dark:text-slate-400 uppercase tracking-wider">Daraja</p>
                  </div>
                  <div className="text-center border-x border-gray-100 dark:border-slate-700/50">
                    ...
                    <p className="text-[10px] font-semibold text-muted-foreground dark:text-slate-400 uppercase tracking-wider">Nishonlar</p>
                  </div>
                  <div className="text-center">
                    <p className="text-3xl font-extrabold text-orange-500 dark:text-orange-400">{userData.streak || 0}</p>
                    <p className="text-[10px] font-semibold text-muted-foreground dark:text-slate-400 uppercase tracking-wider">Seriya</p>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-600 text-white shadow-lg border-none overflow-hidden relative group transition-colors duration-300">
            ...
            <CardContent className="relative z-10 space-y-4">
              ...
              <p className="text-xs md:text-sm text-center text-blue-100 font-medium bg-white/10 py-1.5 rounded-lg">
                Keyingi darajagacha yana <span className="font-bold text-yellow-300">{targetXp - xp} XP</span> kerak
              </p>
            </CardContent>
          </Card>
        </motion.div>
              ...
              <CardHeader className="border-b border-gray-100 dark:border-slate-700/50 pb-4 mb-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                  ...
                  <div className="text-xs md:text-sm font-medium text-muted-foreground dark:text-slate-300 bg-gray-100 dark:bg-slate-700 px-4 py-1.5 rounded-full inline-flex w-fit">
                    Jami olingan: <span className="font-bold text-purple-600 dark:text-purple-400 text-sm md:text-base ml-1">{earnedBadges.length}</span> / {allBadges.length}
                  </div>
                </div>
              </CardHeader>
              <CardContent>
