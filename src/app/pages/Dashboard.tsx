...
export function Dashboard() {
  ...
  const handleVote = async (option: 'A' | 'B') => {
    ...
    // Foydalanuvchi ma'lumotlarini darhol ekranda yangilash
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto min-h-screen bg-slate-50 dark:bg-slate-950 pb-28 w-full overflow-x-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 mt-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl md:text-4xl font-black text-gray-900 dark:text-white tracking-tight">Asosiy Panel</h1>
        </div>
        <p className="text-sm md:text-base font-bold text-slate-500 dark:text-slate-400 mt-1">
          Xush kelibsiz, <span className="text-indigo-600 dark:text-indigo-400">{userData?.first_name || 'Huquqshunos'}</span>! Bugun nimalarni o'rganamiz?
        </p>
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-5 mt-6">
          <Card className="border-0 shadow-md bg-white dark:bg-slate-900 rounded-[20px]">
            <CardContent className="p-4 md:p-5 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest mb-0.5">Daraja</p>
          ...
          <Card className="border-0 shadow-md bg-white dark:bg-slate-900 rounded-[20px]">
            <CardContent className="p-4 md:p-5 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest mb-0.5">Jami XP</p>
          ...
          <Card className="border-0 shadow-md bg-white dark:bg-slate-900 rounded-[20px]">
            <CardContent className="p-4 md:p-5 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest mb-0.5">Seriya</p>
      ...
      {/* --- HAFTA DILEMMASI (SO'ROVNOMA) QISMI --- */}
      {poll ? (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }}>
          <div className="flex items-center gap-2 mb-4 px-2">
            <Clock className="w-5 h-5 text-indigo-500" />
            <h2 className="text-lg md:text-xl font-black text-slate-800 dark:text-white">Hafta Dilemmasi</h2>
          </div>
              ...
              <AnimatePresence mode="wait">
                {!userVote ? (
                  <motion.div key="vote-buttons" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-4 relative">
                    <div className="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-full items-center justify-center z-10 font-black text-slate-300 text-xs">YOKI</div>
                  ...
                  <motion.div key="vote-results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                    <div className="flex items-center justify-center gap-2 mb-6 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 py-2.5 rounded-xl font-bold text-sm">
                      <CheckCircle2 className="w-5 h-5" /> Fikringiz qabul qilindi (+50 XP)
                    </div>
        ...
        <div className="bg-slate-100 dark:bg-slate-800/50 p-10 rounded-[32px] text-center border-2 border-dashed border-slate-200 dark:border-slate-700">
           ...
           <h3 className="text-xl font-bold dark:text-white mb-2">So'rovnoma tez orada</h3>
           <p className="text-slate-500">Yangi haftalik bahsli mavzu hali kiritilmagan.</p>
        </div>
      )}
